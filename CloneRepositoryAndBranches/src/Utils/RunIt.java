...
public class RunIt {
    ...
    public static Output execute(String command, String[] args, String path, boolean getStatus) throws IOException {
        ...
        int exitStatus;
        if (getStatus) {
            //_log.info("Waiting for exit status of " + builder.command().toString() + "; in path: " + builder.directory()); TODO logging
            try {
                exitStatus = proc.waitFor();
            } catch (InterruptedException e) {
                // _log.error("Encountered an interrupt exception while executing " + builder.command().toString() + "; in path: " + builder.directory()); TODO logging
                exitStatus = -1;
            }
        } else
            exitStatus = 0;
...
//      output += "*****-END-OUTPUT-*****\n";
    ...
    static public boolean deleteDirectory(File path) {
        // _log.trace("RunIt::deleteDirectory(..) - deleting " + path); TODO: logging
        if (path.exists()) {
