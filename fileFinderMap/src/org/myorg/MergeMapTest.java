...
public class MergeMapTest {
	...
	public void mapTest(Text value) throws IOException {
		String hostBranchDir, mergeBranchDir, parentBranchDir;
		 String line = value.toString();
	        StringTokenizer tokenizer = new StringTokenizer(line);
	        MergeMapTest mmapHost = new MergeMapTest();
	        MergeMapTest mmapMerge = new MergeMapTest();
	        MergeMapTest mmapParent = new MergeMapTest();
	        ...
	        */
	        ...
	        System.out.println("hostPathNames = " + hostPathNames);
	        System.out.println("hostFileSizes = " + hostFileSizes);
	        ...
	        for (int i=0; i < hostPathNames.size(); i ++){
	        	...
	        	if (mergePathNames.contains(predictedMergePathName)){
	        		if (parentPathNames.contains(predictedParentPathName)){
	        			System.out.println(new Text(currHostPathName + " " + predictedMergePathName + 
	        					" " + predictedParentPathName).toString() + " " +
	        					new LongWritable(Math.max(currHostFileSize, Math.max(currMergeFileSize
	        							, currParentFileSize))).toString() );
	        			...
	        			System.out.println(new Text(currHostPathName + " " + predictedMergePathName + 
	        					" null").toString() + " " +
	        					new LongWritable(Math.max(currHostFileSize, currMergeFileSize)).toString() );
	        		}
     			...
     			mergeFileSizes.remove(mergeBranchIndex);
	        		...
	        		if (parentPathNames.contains(predictedParentPathName)){
	        			System.out.println(new Text(currHostPathName + " " + "null" + 
	        					" " + predictedParentPathName).toString() + " " +
	        					new LongWritable(Math.max(currHostFileSize, currParentFileSize)).toString() );
		        		parentPathNames.remove(parentBranchIndex);
		        		parentFileSizes.remove(parentBranchIndex);
	        		}
	        		else{
	        			System.out.println(new Text(currHostPathName + " null null").toString() + " " + 
	        					new LongWritable(hostFileSizes.get(i)).toString() );
	        		}
	        	}
	        }
	        ...
	        for (int i = 0 ; i < mergePathNames.size(); i ++){
	        	...
	        	if (parentPathNames.contains(predictedParentPathName)){
	        		System.out.println(new Text("null " + currMergePathName + 
     					" " + predictedParentPathName).toString() + " " + 
     					new LongWritable(Math.max(currMergeFileSize, currParentFileSize)).toString() );
	        		parentPathNames.remove(parentBranchIndex);
	        		parentFileSizes.remove(parentBranchIndex);
     		}
     		else{
     			System.out.println(new Text("null " + currMergePathName + " null").toString()+ " " + 
     					new LongWritable(currMergeFileSize).toString() );
     		}
	        }
	        
	        for (int i = 0 ; i < mergePathNames.size(); i ++){
	        	System.out.println(new Text("null null " + parentPathNames.get(i)).toString() + " " +  
     					new LongWritable(parentFileSizes.get(i)).toString() );
	        }
...
/*		  JobConf conf = new JobConf(MergeMapTest.class);
	      conf.setJobName("MergeMapTest1");
	...
	*/
		...
		MergeMapTest mer = new MergeMapTest();
		mer.mapTest(new Text("hostBranch mergeBranch parentBranch"));//recursive_ls("/user/hamid_bably/gitinput/");
